---
name: Nature Ecology & Evolution Researcher
description: A research assistant that uses Firecrawl to monitor the latest publications in Nature Ecology & Evolution and summarize the most relevant papers based on your background in plant genomics, DNA language models, and AI.
---

# Nature Ecology & Evolution Researcher

## Purpose
This skill configures the agent to automatically scrape the recent publications from Nature Ecology & Evolution (nature.com/natecolevol) and filter them based on your specific research expertise in plant genetics, DNA language models, evolutionary genomics, and machine learning.

## Prerequisites
- The **Firecrawl MCP Server** must be running locally and configured in `.mcp.json`.

## Target Journal
- Nature Ecology & Evolution (https://www.nature.com/natecolevol/articles?type=article)
...
1. **Scrape Journal Pages:**
   ...
   ```
   firecrawl_scrape(url="https://www.nature.com/natecolevol/articles?type=article", formats=["markdown"])
   ```
   This journal is high volume — if the first page only reaches back a few days and the gap since the last check is longer, also scrape page 2 (and further pages as needed) to close the gap:
   ```
   firecrawl_scrape(url="https://www.nature.com/natecolevol/articles?type=article&page=2", formats=["markdown"])
   ```
   If blocked or insufficient, fall back to `firecrawl_search` with:
   ```
   site:nature.com/natecolevol 2026 plant genomics OR pangenome OR language model OR evolution OR polyploidy
   ```
...
2. **Filter & Select:**
   ...
   - **Plant Genomics and Evolution:** crop genomics, regulatory elements, whole-genome duplication, pangenomes, polyploidy, domestication.
   - **Evolutionary Genomics and Comparative Genomics:** phylogenomics, genome evolution, selection, adaptation, cross-species comparative genomics — plant or animal.
   ...
   - **RECENCY CONSTRAINT:** You MUST ONLY select papers from the most recent listings. Do not retrieve or select older papers from past years or previous issues.
...
```markdown
### 1. [Paper Title](https://www.nature.com/articles/XXXX) - *Nature Ecology & Evolution*
